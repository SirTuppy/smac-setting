...
export type AppView = 'mission-control' | 'analytics' | 'mapper' | 'report' | 'shift-analyzer' | 'wsp-generator' | 'budget-tracker' | 'setter-path';
...
export interface GymBudgetConfig {
  annualBudget: number;
  categoryLimits: Record<string, number>; // Mapping categoryId -> annualLimit
  yearlyLimits?: Record<number, { annualBudget: number; categoryLimits: Record<string, number> }>;
}
...
export type MilestoneCategory = 'Movement' | 'Coordination' | 'Workflow' | 'Maintenance' | 'Safety' | 'Aesthetics' | 'Speed' | 'Other';
export type MilestoneStatus = 'locked' | 'available' | 'review' | 'completed';
...
export interface MilestoneTemplate {
  ...
  libraryId?: string; // Link to SetterLibrary section
}
...
export interface PhaseTemplate {
  ...
  description: string;
  milestoneIds: string[];
  exitCriteriaDescriptions: string[];
}
...
export interface ApprenticeInstance {
  setterName: string;
  frameworkId: string;
